...
from descriptors import load_gpt_descriptions, wordify
import pathlib
...
from datasets import _transform, CUBDataset, _transform_origin, RemappedImageFolder
...
hparams['category_name_inclusion'] = None #'append' 'prepend'
...
tfms_origin = _transform_origin(hparams['image_size'])
...
if hparams['dataset'] == 'imagenet':
    if hparams['dataset'] == 'imagenet':
        ...
        dataset = dsclass(hparams['data_dir'], split='train', transform=tfms)
        classes_to_load = None
    
        if hparams['descriptor_fname'] is None:
            hparams['descriptor_fname'] = 'my_imagenet.json'
            # hparams['descriptor_fname'] = 'my_imagenet_v1.json'
            # hparams['descriptor_fname'] = 'descriptors_imagenet.json'
    ...
    hparams['descriptor_fname'] = 'my_cub.json'

elif hparams['dataset'] == 'eurosat':
    # load EuroSAT dataset
    hparams['data_dir'] = EUROSAT_DIR
    entire_dataset = torchvision.datasets.ImageFolder(hparams['data_dir'], transform=tfms)
    classes_to_load = None #dataset.classes
    hparams['descriptor_fname'] = 'my_eurosat.json'
    train_set, test_set = torch.utils.data.random_split(entire_dataset, [24000, 3000])
    dataset = train_set

elif hparams['dataset'] == 'oxfordpet':
    # load OxfordPet dataset
    hparams['data_dir'] = OXFORDPET_DIR
    dataset = torchvision.datasets.OxfordIIITPet(root=hparams['data_dir'], transform=tfms, split='trainval')
    classes_to_load = None #dataset.classes
    hparams['descriptor_fname'] = 'my_oxfordpet.json'

elif hparams['dataset'] == 'food101':
    # load Food-101 dataset
    hparams['data_dir'] = FOOD101_DIR
    dataset = torchvision.datasets.Food101(root=hparams['data_dir'], transform=tfms, split='train')
    classes_to_load = None #dataset.classes
    hparams['descriptor_fname'] = 'my_food101.json'
...
def compute_description_encodings(model):
    description_encodings = OrderedDict()
    for k, v in gpt_descriptions.items():
        tokens = clip.tokenize(v[:5]).to(hparams['device'])
        description_encodings[k] = F.normalize(model.encode_text(tokens))
    return description_encodings
###########################
